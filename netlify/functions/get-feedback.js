...
exports.handler = async function(event) {
    ...
    try {
        ...
        // === 수정된 부분 시작 ===
        // OpenAI로부터 받은 응답이 정상이 아닐 경우, 에러를 발생시켜 catch 블록으로 넘깁니다.
        if (!response.ok) {
            const errorBody = await response.json().catch(() => ({ error: 'Failed to parse error JSON from OpenAI' }));
        ...
        // === 수정된 부분 시작 ===
        // 발생한 모든 종류의 오류를 상세하게 기록합니다.
        console.error('Full Error Details in Catch Block:', error);
        ...
        return {
            ...
            body: JSON.stringify({ 
                ...
                details: error.message // 오류의 원인을 클라이언트에서도 확인할 수 있도록 추가
            })
        };
        // === 수정된 부분 끝 ===
    }
